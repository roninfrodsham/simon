...
function App() {
  ...
  async function lightUpColours() {
    ...
    const newGameSpeed =
      gameState.speed - 100 < 200 ? 200 : gameState.speed - 100;
  ...
  async function gameButtonClickHandle(selectedColour) {
    if (!gameState.simonMode && gameState.userPlay) {
      ...
      if (selectedColour === colour) {
        ...
        console.log("FAIL");
        errorSound.play();
        setFinalScore(gameState.colours.length - 1);
        await timeout(500);
        setGameState({
          ...initialGameState,
          simonMode: false,
  ...
  return (
    <div className="simon circle oh bGray">
      ...
      <div className={`gameOver circle bGray cc ${gameOver ? "show" : ""}`}>
        <h1>LEVEL {finalScore}</h1>
        ...
        />
        <div className="submit" onClick={sendToLeaderboard}>
          Submit
        </div>
      </div>
    </div>
